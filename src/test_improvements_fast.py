...
def test_behavior_algorithm_v2_structure():
    """测试行为算法V2结构（不加载模型）"""
    print("\n" + "="*60)
    print("测试 1: 行为算法升级 - 结构验证")
    print("="*60)
    ...
    try:
        from algorithms.behavior_alert_v2 import (
            ...
            ClimbingDetectionAlgorithm
        )
        
        print("✓ 模块导入成功")
        
        pose_estimator = PoseEstimator()
        print(f"✓ PoseEstimator 初始化: 关键点数量={len(pose_estimator.keypoint_names)}")
        
        fall_detector = FallDetectionAlgorithmV2()
        print(f"✓ FallDetectionAlgorithmV2 初始化: 角度阈值={fall_detector.fall_angle_threshold}")
        
        frame = np.random.randint(0, 255, (720, 1280, 3), dtype=np.uint8)
        result = fall_detector.process(frame)
        
        print(f"✓ 摔倒检测回退模式测试: 检测={result.detected}, 置信度={result.confidence:.2f}")
...
def test_database_optimization():
    ...
    try:
        ...
        print("✓ SQLite 数据库初始化成功 (WAL模式已启用)")
...
def test_shared_state():
    ...
    try:
        from shared_state import SharedStateManager, FileStateManager, RedisStateManager
...
def main():
    """运行所有测试"""
    print("\n" + "="*60)
    print("AI Box v1.8.0 改进验证测试（快速版）")
    print("="*60)
    
    tests = [
        ("行为算法升级 (YOLOv8)", test_behavior_algorithm_v2_structure),
